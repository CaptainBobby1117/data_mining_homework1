...
import sys
#导出控制台输出到日志文件中
log_print = open('Defalust.log', 'w',encoding='utf8')
...
print(data)
for column_name in ['IMDb-rating','appropriate_for','director','downloads','id','industry','language','posted_date','release_date','run_time','storyline','title','views','writer']:
...
for i in range(4):
    if(i==3):
        data_tmp1 = data.drop(['appropriate_for','director','downloads','industry','language','posted_date','release_date','run_time','storyline','title','views','writer'], axis=1)
    ...
    column_name_for_rating=['IMDb-rating','id']
    ...
    for column_name in ['IMDb-rating','appropriate_for','director','downloads','id','industry','language','posted_date','release_date','run_time','storyline','title','views','writer']:
        ...
        # 通过数据对象之间的相似性来填补缺失值
            ...
            data = pd.DataFrame(imputer.fit_transform(data_tmp.values),columns=['IMDb-rating','id'])
            print(data)
